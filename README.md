...
- 同一告警在冷却时间内不重复推送
- 状态恢复时会推送 “已恢复”
- 每条规则会维护一个 active index，用于对比上一轮仍在告警的事件；恢复检测不需要每轮 `list()` KV keys
- 已恢复记录会在 `max(cooldown * 2, 1h)` 后自动过期，避免 KV 堆积

升级到 active index 版本后，已经存在且仍在 firing 的告警会在下一次巡检时写入 index，之后可正常发送恢复通知。升级前已经恢复、但只存在旧单条状态 key 的告警不会再通过 `list()` 回扫补发恢复通知。
